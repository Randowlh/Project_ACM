    ...
    int getrank(int val){
        ...
        while(now){
            if(val<=tzy[now].val)
                ...
                rk+=tzy[now].exsit+tzy[tzy[now].l].fact;
                now=tzy[now].r;
            }
        }
        return rk;
...
signed main()
{
    ...
    for(int q=0;q<t;q++){
        ...
        // tree.tt.clear();
        // tree.zhongxu(tree.root );
        // cout<<"tt=";
        // for(int i=0;i<tree.tt.size();i++){
        //     cout<<tree.tt[i]<<' '<<tree.tzy[tree.tt[i]].val<<' '<<endl;
        // }
        // for(int i=1;i<=tree.cnt;i++){
        //     cout<<"i="<<tree.tzy[i].val<<' '<<tree.tzy[i].l<<' '<<tree.tzy[i].r<<endl;
        // }
        // cout<<endl<<endl;
    }
}